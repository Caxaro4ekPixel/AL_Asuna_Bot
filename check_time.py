                    ...
                    if relese:
                        if relese[3]:
                            ...
                            if days >= 0:
                                ...
                                if temp:
                                    cur.execute(
                                        f'''update results set time = "{(timer.days * 86400) + timer.seconds}", last_up = {last_up} where id = {temp[0]}''')
                                else:
                                    cur.execute(
                                        f'''insert into results ('id', 'chat', 'relese', 'time', 'last_up') values ({i["id"]} ,{relese[0]}, "{relese[5]}", "{(timer.days * 86400) + timer.seconds}", {i["updated"]})''')
                                ...
                                if last_ser == all_ser:
                                    ...
                                    bot.send_message(chat_id=relese[0],
                                                     text=f"Релиз завершён! Всем спасибо за работу! Этот чат более не активен.")
        else:
